...
- В чём разница между наблюдениями в городах и сельской местности?
...
Наблюдения были сгруппированы по годам и месяцам, а затем занесены в сводную таблицу [[19]]. С помощью Python и Pyodbc создан barplot хронологического распределения случаев.
...
- Наблюдения примерно равномерно разделились  по исходам попытки суицида (1315, 51.15% - выжили; 1256, 48.85% - умерли) [[]]
- Видны сезонные флуктуации случаев. Летом фиксируется максимум, зимой - минимум
- Все выжившие попытку суицида были госпитализированы. Из впоследствии умерших, госпитализировано лишь 238, 19% [[]]. Исход попытки - главный предиктор госпитализации
...
- Наиболее распространённые методы: употребление пестицида (1768, 68.77%), повешение (431, 16.76%), другие яды (146, 9.84%); наименее: прыгание (15, 0.58%), утопление (26, 1.01%), порезы (29, 1.13%) 
- Наиболее смертельные методы: утопление (26/26, 100%), повешение (419/431, 97%), прыгание (12/15, 80%); наименее: неопределённый яд (3/104, 3%), неопределённый (3/48, 6%), другой яд (15/146, 10%) [[]]
- Наиболее распространённые методы женщин: другой яд (66% наблюдений использования относится к женщинам), утопление (68%), пестицид (54%); мужчин: неопределённый (63% наблюдений использования относится к мужчинам), повешение (61%), порезы (52%) [[]]
...
Для определения связи категорических значений был использован Хи-тест независимости категорий. Составлена тепловая карта pvalue для полей со значениями-категориями. Алгоритм визуализации заимствован у [Shafqaat Ahmad](https://medium.com/analytics-vidhya/constructing-heat-map-for-chi-square-test-of-independence-6d78aa2b140f), ([github](https://github.com/shafqaatahmad/chisquare-test-heatmap/tree/main)).
...
Тип данных датасета оставил возможность построения только моделей логистической регрессии. В качестве целевого значения предсказаний использовался исход суицида.
...
Гипотеза 1. Уровень образования - прокси-значение по отношению к проживанию человека в городе или сельской местности. Опровергается низкой важностью местности в регрессии, а также относительно невысокой разницей смертельности попыток самоубийств между городом и деревней. Хотя влияние местности невозможно исключить (ГИПОТЕЗА!!), его не следует считать решающим.

Гипотеза 2. Предыдущие наблюдения свидетельствовали о росте смертельности попыток суицида с увеличением возраста. Совокупность начальных ступеней образования и высокого возраста - черта поколения, родившегося в годы индустриализации и большого скачка (1950-е - 1960-е гг). Опровергается низкой важностью возраста в регрессии, а также тем, что у возрастных групп 60-80 лет фиксируется один из наиболее высоких удельных весов образования выше начального.
